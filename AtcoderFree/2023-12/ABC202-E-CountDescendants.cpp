...
struct Solver {
  void solve() {
    ...
    int cnt = 0;
    auto dfs = [&](auto rec, int v = 0) -> void {
      inIdx[v] = cnt;
      for (auto u : G[v]) {
        depth[u] = depth[v] + 1;
        ++cnt;
    ...
    for (int i = N - 1; i >= 0; i--)
      mp[depth[i]].push_back(inIdx[i]), mp[depth[i]].push_back(outIdx[i]);
    ...
    rep(qi, Q) {
      ...
      debug(mp[D], inIdx[U], outIdx[U]);

      auto it1 = lower_bound(all(mp[D]), inIdx[U]);
      auto it2 = upper_bound(all(mp[D]), outIdx[U]);

      debug(distance(mp[D].begin(), it1), distance(mp[D].begin(), it2));

      out(distance(it1, it2) / 2);
    }
