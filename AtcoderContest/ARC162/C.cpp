...
struct Solver {
  void solve() {
    ...
    rep(ti, T) {
      ...
      int flag = 0;
      pair<int, set<int>> dummy;

      auto dfs = [&](auto rec, int v) -> pair<int, set<int>> {
        if (flag) return dummy;
        set<int> ret;
        int retv = 0;
        for (auto u : G[v]) {
          auto p = rec(rec, u);
          if (flag) break;
          retv += p.first;
          for (auto a : p.second) ret.insert(a);
        }
        if (A[v] == -1)
          retv++;
        else if (A[v] <= K)
        ...
        if (ret.count(K) == 0) {
          if (retv == 0 && (int)ret.size() == K) flag = 1;
          if (retv == 1 && (int)ret.size() >= K - 1) flag = 1;
        }

        return make_pair(retv, ret);
      };
