...
struct Solver {
  ...
  void solve() {
    ...
    /* 焼きなまし法 */
    const static double START_TEMP = 1000;  // 開始時の温度
    const static double END_TEMP = 10;      // 終了時の温度
    const static double END_TIME = 10;      // 終了時間（秒）
    ...
    do {
      ...
      vc beforeStatusD(N);
      rep(i, N) beforeStatusD[i] = status[i][d];
      ...
      if (probability < rand01()) {
        ...
        maxCut1[d] = BeforeP1;
        maxCut2[d] = BeforeP2;
        rep(i, N) status[i][d] = beforeStatusD[i];
      } else {
        maxScore = nowScore;
      }
