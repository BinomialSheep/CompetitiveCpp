...
struct Solver {
  ...
  void calcStatus(vector<Point> &cut1, vector<Point> &cut2, int num) {
    rep(i, N) {
      rep(j, num) {
        int res = Point::ccw(cut1[j], cut2[j], points[i]);
        // if (i < 10 & j < 1) {
        //   debug(cut1[j].x, cut1[j].y, cut2[j].x, cut2[j].y);
        //   debug(points[i].x, points[i].y);
        //   debug(res);
        // }
        if (res == 1) {
          status[i][j] = '1';
  ...
  void changeStatus(int d, vector<Point> &cut1, vector<Point> &cut2) {
    //
    rep(i, N) {
      int res = Point::ccw(cut1[d], cut2[d], points[i]);
      if (res == 1) {
        status[i][d] = '1';
      } else if (res == -1) {
        status[i][d] = '0';
  ...
  void calcStrawberry() {
    ...
    rep(i, N) mp[status[i]]++;
    for (auto p : mp) {
      rep(i, p.first.size()) if (p.first[i] == '2') continue;
      if (p.second <= 10) strawberry[p.second]++;
    }
    // debug(strawberry);
  }

  void smartChangeStatus(int d, vector<Point> &cut1, vector<Point> &cut2) {
    rep(i, N) {
      char before = status[i][d];
      char after;
      int res = Point::ccw(cut1[d], cut2[d], points[i]);
      if (res == 1) {
        after = '1';
  ...
  double calcScore() {
    double score = 0;
    rep(i, 10) { score += min(a[i], strawberry[i + 1]) / (double)a[i]; }
    // debug(score);
    return score;
  }
  ...
  void StupidInit(vector<Point> &cut1, vector<Point> &cut2, int num) {
    rep(i, num) {
      cut1[i] = Point(i + 1, i + 1);
      cut2[i] = Point(-i, -i);
    }
  }

  void randomOne(Point &p1, Point &p2) {
    int RANGE = 1e4;
    int x = randXor() % RANGE;
    int y = randXor() % RANGE;
    if (x % 2) x = -x;
  ...
  void solve() {
    ...
    // 初期解
    double maxScore = 0;
    vector<Point> maxCut1(K);
    vector<Point> maxCut2(K);
    int maxNum = CUT_NUM;

    resetSt();
    StupidInit(maxCut1, maxCut2, CUT_NUM);
    calcStatus(maxCut1, maxCut2, CUT_NUM);
    calcStrawberry();
    maxScore = calcScore();

    // debug(((double)duration_cast<microseconds>(system_clock::now() -
    // startClock)
    //            .count() *
    //        1e-6));
    ...
    /* 焼きなまし法 */
    const static double START_TEMP = 100;  // 開始時の温度
    const static double END_TEMP = 1;      // 終了時の温度
    const static double END_TIME = 2.6;    // 終了時間（秒）
    ...
    do {
      ...
      // d個目の切り方をtx1, ty1, tx2, ty2に変えてみる
      // int d = randXor() % CUT_NUM;
      int d = cnt % CUT_NUM;
      cnt++;
      Point p1, p2;
      randomOne(p1, p2);
      // バックアップ
      double beforeScore = maxScore;
      Point BeforeP1 = maxCut1[d];
      Point BeforeP2 = maxCut2[d];

      // 変えてみる
      maxCut1[d] = p1;
      maxCut2[d] = p2;
      // changeStatus(d, maxCut1, maxCut2);
      // calcStrawberry();
      smartChangeStatus(d, maxCut1, maxCut2);
      double nowScore = calcScore();
      // debug(strawberry);
      // debug(beforeScore, nowScore);

      //  焼きなまし法の遷移確率
      double deltaScore = (nowScore - beforeScore) * 1000;
    ...
    // 出力
    // debug(time);
    print(maxCut1, maxCut2, maxNum);
    // debug(cnt);
    //   提出時は消す
    //   resetSt();
    //   calcStatus(maxCut1, maxCut2, maxNum);
    //   calcStrawberry();
    //   debug(calcScore());
    //   debug(strawberry);
