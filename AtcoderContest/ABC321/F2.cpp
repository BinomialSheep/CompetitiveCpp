...
struct Solver {
  void solve() {
    ...
    vs A(Q);
    vi B(Q);
    rep(qi, Q) cin >> A[qi] >> B[qi];

    int mx = 0;
    // rep(qi, Q) if (A[qi] == "-") mx += B[qi];
    // mx += K + 1;
    // debug(mx);
    rep(qi, Q) mx += B[qi];
    mx = K;
    ...
    rep(qi, Q) {
      ...
      if (ty == "+") {
        ...
        for (auto it = dp.rbegin(); it != dp.rend(); it++) {
          auto [k, v] = *it;
          // debug(k);
          auto tmp = dp[k] - (dp2.count(k) ? dp2[k] : 0);
          if (tmp.val()) dp2[k - x] = dp[k] - dp2[k];
        }
      }
      swap(dp, dp2);
      out(dp.count(K) ? dp[K].val() : 0);

      // for (auto [k, v] : dp) debug(qi, k, v.val());
    }
