...
struct Solver {
  void solve() {
    ...
    rep(i, M) {
      if (S[A[i]] == C[i]) G[A[i]].emplace_back(B[i], C[i]);
      if (S[B[i]] == C[i]) G[B[i]].emplace_back(A[i], C[i]);
      if (C[i] == 'B') {
        mp[make_pair(A[i], B[i])] = i + 1;
        mp[make_pair(B[i], A[i])] = i + 1;
      } else {
        mp[make_pair(-A[i], B[i])] = i + 1;
        mp[make_pair(-B[i], A[i])] = i + 1;
      }
    ...
    auto vu2e = [&](int v, int u, int c) {
      if (c == 'B') return mp[make_pair(v, u)];
      return mp[make_pair(-v, u)];
    };
    ...
    rep(v, N) {
      if (G[v].size() == 1) {
        ...
        if (S[v] != S[u] && dc.same(v, G[v][0].first)) {
          out("No");
          return;
        }
        if (st2.count(v) == 0) {
          ans.push_back(vu2e(v, u, G[v][0].second));
          st2.insert(v);
          dc.link(v, u);
          debug(v, u, S[v], S[u], G[v][0].second, S[u] == G[v][0].second);
          if (S[u] == G[v][0].second) st2.insert(u);
        }
    ...
    auto dfs = [&](auto rec, int v) -> bool {
      ...
      double time =
          ((double)duration_cast<microseconds>(system_clock::now() - startClock)
               .count() *
           1e-6);
      const static double END_TIME = 1.2;
      if (time > END_TIME) {
        // timeover = 1;
        return false;
      }
