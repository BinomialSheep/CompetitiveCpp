...
bool stupid(vl& A, int l, int r, ll K) {
  debug(A);
  ...
  debug(A);
  bool ret = true;
  rep(i, K - 1) if (A[l + i] != A[l + i + 1]) ret = false;
  return ret;
}
...
struct Solver {
  void solve() {
    ...
    rep(j, N) {
      rep(i, K) prefix[i][j + 1] = prefix[i][j];
      prefix[j % K][j + 1] += B[j];
    ...
    rep(qi, Q) {
      ...
      if (r - l <= 100) {
        vl C;
        --r;
        for (int i = l + 1; i <= r + 1; i++) C.push_back(A[i]);
        Yes(stupid(C, 0, C.size() - 1, K));
        continue;
      }
      ...
      rep(i, K) {
        ...
        // while (r % K != i) r--;
        //
        debug(left, right, prefix[i][left], prefix[i][right]);
        if (prefix[i][left] != prefix[i][right]) ans = 0;
      }
      // rep(i, K) {
      //   int j = (l + i) % K;
      //   if (prefix[j][r] != prefix[j][l]) ans = 0;
      // }
