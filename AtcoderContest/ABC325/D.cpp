...
struct Solver {
  void solve() {
    ...
    /* solve */
    deque<pair<ll, int>> qin;
    deque<pair<ll, int>> qout;
    rep(i, N) qin.emplace_back(T[i], i);
    // rep(i, N) qout.emplace_back(T[i] + D[i], i);
    sort(all(qin));
    // sort(all(qout));
    int cnt = 0;
    ll now = 0;
    ...
    while (qin.size() || pq.size()) {
      while (qin.size() && now >= qin.front().first) {
        auto [f, s] = qin.front();
        qin.pop_front();
        pq.emplace(f + D[s], s);
      }
      ...
      if (pq.size()) {
        ...
        now = qin.front().first;
        // ll tmp1 = qin.size() ? qin.front().first : (ll)3e18;
        // now = tmp1;
        // ll tmp2 = qout.size() ? qout.front().first : (ll)3e18;
        // now = min(tmp1, tmp2);
      }
    }
    out(cnt);
