...
struct Solver {
  void solve() {
    ...
    auto f = [&](auto rec, string t) -> void {
      ...
      rep(i, (int)t.size() - 1) {
        if (t[i] == 'o' && t[i + 1] == 'f') {
          ...
          if (idx == -1 || idx - i + 1 > K) {
            if (i + 2 + K < t.size())
              rec(rec, t.substr(0, i) + t.substr(i + 2 + K));
            else
              rec(rec, t.substr(0, i));
          } else {
            rec(rec, t.substr(0, i) + t.substr(idx));
            if (idx + 1 < t.size())
              rec(rec, t.substr(0, i) + t.substr(idx + 1));
            if (i + 2 + K < t.size())
              rec(rec, t.substr(0, i) + t.substr(i + 2 + K));
            else
              rec(rec, t.substr(0, i));
    ...
    int ans = (int)S.size();
    for (auto t : st) chmin(ans, (int)t.size());
    out(ans);
