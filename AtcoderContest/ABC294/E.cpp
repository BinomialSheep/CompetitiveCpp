...
struct Solver {
  void solve() {
    ...
    while (A.size() || B.size()) {
      // 短い方まで進める
      ll tmp = min(A.front().second, B.front().second);
      if (A.front().first == B.front().first) ans += tmp;
      B.front().second -= tmp;
      A.front().second -= tmp;
      if (A.front().second == 0) A.pop_front();
      if (B.front().second == 0) B.pop_front();
    }
    out(ans);
