...
// 重軽分解（Heavy Light Decomposition）
// beet-aizuさんから窃盗
...
class HLD {
 ...
 public:
  ...
  // G:=普通にグラフだけどbuild時に親方向への辺を消す
  // vid:=同じ連結成分が直線に並ぶ nxt:=連結成分の代表値の番号？
  // sub:=自身を含む部分木サイズ par:=親 inv:=vidのinv
...
struct Solver {
  void solve() {
    ...
    rep(i, N - 1) {
      int z = G.lca(U[i], V[i]);
      assert(z == U[i] || z == V[i]);
      G.for_each_edge(z, U[i] + V[i] - z, [&](int l, int r) {
        assert(idx[i] == -1);
        assert(r - l == 1);
        idx[i] = l;
        BIT.add(l, w[i]);
    ...
    rep(qi, Q) {
      ...
      if (op == 1) {
        ...
        ll ans = 0;
        int z = G.lca(u, v);
        G.for_each_edge(z, u, [&](int l, int r) { ans += BIT.sum(l, r); });
        G.for_each_edge(z, v, [&](int l, int r) { ans += BIT.sum(l, r); });
        cout << ans << '\n';
      }
    }
