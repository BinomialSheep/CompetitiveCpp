...
struct Solver {
  void solve() {
    ...
    auto f = [&](long double mid) {
      ...
      rep(i, N) C.emplace_back(A[i].first - A[i].second * (mid / (1 - mid)));
      rep(i, M) D.emplace_back(B[i].first - B[i].second * (mid / (1 - mid)));
      sort(all(D));
      // sort(C.rbegin(), C.rend());
      // debug(mid, C, D);

      rep(i, N) cnt += M - (lower_bound(all(D), -C[i]) - D.begin());
      ...
      //   int y1 = A[i].second;
      //   int y2 = B[j].second;
      //   long double tmp = 100 * (x1 + x2) / (long double)(x1 + x2 + y1 + y2);
      //   if (mid <= tmp) cnt++;
      // }
      // debug(mid, cnt);
