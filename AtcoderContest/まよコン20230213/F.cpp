    ...
    rep(i, N) {
      ...
      if (S[i] == "AB") {
        ...
        if (A > B) {
          ...
          auto it1 = lower_bound(all(ac), i);
          auto it2 = lower_bound(all(bc), i);
          if (it1 == ac.end()) {
            A--;
            B++;
          } else if (it2 == bc.end()) {
      ...
      debug(tmp, A, B, C);
      if (tmp[0] < A) ans.push_back("A");
      if (tmp[1] < B) ans.push_back("B");
      if (tmp[2] < C) ans.push_back("C");
      assert(A >= 0 && B >= 0 && C >= 0);
    }
    out("Yes");
    out(ans);
