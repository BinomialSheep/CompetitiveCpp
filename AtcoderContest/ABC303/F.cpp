...
struct Solver {
  void solve() {
    ...
    auto isOK = [&](ll n) {
      priority_queue<pair<ll, ll>> pq2 = pq;
      ll h = H;
      ...
      while (t <= n) {
        ...
        ll tmp = (n - p.second + 1) - t + 1;
        // debug(tmp);
        if (tmp >= 1) {
          // if (tmp <= 0) return false;
          if (tmp > (1e18 / p.first) + 1) return true;
          h -= tmp * p.first;
        ...
        ll nokori = n - t + 1;
        if (nokori > 0) {
          // debug(p.first / p.second * nokori, nokori);
          pq2.emplace(p.first / p.second * nokori, nokori);
        }
      }
