...
struct Solver {
  void solve() {
    ...
    // // +を何個入れていいか
    // int cntplus = 0;
    // rep(i, 2 * N) if (T[i] == '+') cntplus++;
    // // N - cntplus個数える
    int sz = 2 * N + 2;
    vector<pair<mint, queue<int>>> dp(2 * N + 2);
    dp[N].first = 1;

    rep(i, 2 * N) {
      vector<pair<mint, queue<int>>> p(2 * N + 2);
      //
      if (T[i] == '+') {
        rep(j, sz - 1) {
          p[j + 1] = dp[j];
          if (dp[j].second.empty() || dp[j].second.back() > 0) {
            p[j + 1].second.push(i + 1);
          } else {
            int v = -p[j + 1].second.back();
            p[j + 1].second.pop();
            p[j + 1].first += i - v;
          }
        }
