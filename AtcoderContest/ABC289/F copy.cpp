...
struct Solver {
  void solve() {
    ...
    rep(i, 10) {
      dp[i + 1][0] = 2 * a - dp[i][0];
      chmin(dp[i + 1][0], 2 * a - dp[i][1]);
      dp[i + 1][1] = 2 * b - dp[i][1];
      chmax(dp[i + 1][1], 2 * b - dp[i][0]);
      dp[i + 1][2] = 2 * c - dp[i][2];
      chmin(dp[i + 1][2], 2 * c - dp[i][3]);
      dp[i + 1][3] = 2 * d - dp[i][3];
      chmax(dp[i + 1][3], 2 * d - dp[i][2]);
      if (dp[i + 1][0] <= tx && tx <= dp[i + 1][1]) {
        if (dp[i + 1][2] <= ty && ty <= dp[i + 1][3]) {
          cnt = i + 1;
          break;
        }
      }
