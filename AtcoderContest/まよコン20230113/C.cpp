...
struct Solver {
  void solve() {
    ...
    ll ans = 0;
    rep(b, N) rep(a, b) {
      // auto it = upper_bound(all(L), L[b] - L[a]);
      // if (it == L.end()) continue;
      auto it2 = lower_bound(all(L), L[b] + L[a]);
      if (it2 == L.begin()) continue;
      --it2;
      // debug(distance(L.begin(), it));
      debug(distance(L.begin(), it2));
      ans += distance(L.begin(), it2) - b;
    }
    out(ans);
