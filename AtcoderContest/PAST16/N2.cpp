...
using S = pair<mint, int>;  // 型エイリアス
// 二項演算S op(S a, S b)
S op(S a, S b) {
  S ret = {mint(0), 0};
  if (a.second && b.second) {
    ret.first = a.first + b.first * mint(K).pow(a.second);
...
struct Solver {
  void solve() {
    ...
    vs ty(Q);
    vi X(Q);
    rep(i, Q) cin >> ty[i] >> X[i];
    vi A;
    rep(i, Q) if (ty[i] == "+") A.push_back(X[i]);
    sort(all(A));
    int N = (int)A.size();
    map<int, int> mp;
    rep(i, N) if (mp.count(A[i]) == 0) mp[A[i]] = i;

    /* solve */
    debug(A, mp);

    segtree<S, op, e> seg(N);  // A(N, e)で初期化
    rep(i, N) seg.set(i, {mint(A[i]), 0});

    auto printSeg = [&](int qi = -1) {
      debug(qi);
      rep(i, N) {
        auto res = seg.get(i);
        debug(i, res.first.val(), res.second);
      }
    };

    rep(qi, Q) {
      if (ty[qi] == "+") {
        seg.set(mp[X[qi]], {mint(X[qi]), 1});
        mp[X[qi]]++;
      } else {
        mp[X[qi]]--;
        seg.set(mp[X[qi]], {mint(X[qi]), 0});
      }
      // debug(mp);
      // printSeg(qi);
      S res = seg.all_prod();
