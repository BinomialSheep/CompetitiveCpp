...
Path initDfs2(Cells& cells) {
  ...
  Path path;
  vector visited(N, vi(N));
  vvi serachNow(N, vi(N));

  auto dfs = [&](auto rec, int y = 0, int x = 0) -> int {
    serachNow[y][x] = 1;
    ...
    vector<pair<int, int>> explorable;
    if (cells[y][x].up && !visited[y - 1][x])
      explorable.emplace_back(cells[y - 1][x].d, UP);
    if (cells[y][x].down && !visited[y + 1][x])
      explorable.emplace_back(cells[y + 1][x].d, DOWN);
    if (cells[y][x].left && !visited[y][x - 1])
      explorable.emplace_back(cells[y][x - 1].d, LEFT);
    if (cells[y][x].right && !visited[y][x + 1])
      explorable.emplace_back(cells[y][x + 1].d, RIGHT);
    sort(explorable.rbegin(), explorable.rend());

    for (auto [_, d] : explorable) {
    ...
    if (ret == -1 && (int)explorable.size() == 0 && (int)path.size() >= 3) {
      ...
      if (cells[y][x].right && serachNow[y][x + 1])
        movableSet.emplace(y, x + 1);
      if (movableSet.empty()) {
        serachNow[y][x] = 0;
        return -1;
      }
