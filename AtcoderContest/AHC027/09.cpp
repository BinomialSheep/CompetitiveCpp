...
int dfs(Cells& cells, Path& path, vvi& visited, vvi& serachNow, int y, int x,
        int playout, Path& homePath) {
  serachNow[y][x] = 1;
  int ret = -1;
  auto f = [&](int dy, int dx, int playout2) {
    pushBackCell(cells, path, y + dy, x + dx);
    visited[y + dy][x + dx] = true;
    int res = dfs(cells, path, visited, serachNow, y + dy, x + dx, playout2,
                  homePath);
    if (res == -1) {
  ...
  if (playout == 1 || explorable.size() <= 1) {
    for (auto d : explorable) {
      if (d == UP && !visited[y - 1][x]) f(-1, 0, playout);
      if (d == DOWN && !visited[y + 1][x]) f(1, 0, playout);
      if (d == LEFT && !visited[y][x - 1]) f(0, -1, playout);
      if (d == RIGHT && !visited[y][x + 1]) f(0, 1, playout);
    }
    // } else if (playout == -1) {
    //   for (auto d : explorable) {
    //     if (d == UP && !visited[y - 1][x]) f(-1, 0, 0);
    //     if (d == DOWN && !visited[y + 1][x]) f(1, 0, 0);
    //     if (d == LEFT && !visited[y][x - 1]) f(0, -1, 0);
    //     if (d == RIGHT && !visited[y][x + 1]) f(0, 1, 0);
    //   }
  } else {
    vvi nowVisited = visited;
    Path nowPath = path;
    Path nowHomePath = homePath;
    Cells nowCells = cells;
    vvi nowSerachNow = serachNow;
    int nowRet = ret;
    ...
    vi perm(explorable.size());
    iota(all(perm), 0);
    do {
      rep(i, explorable.size()) {
        // int d = explorable[perm[i]];
        int d = explorable[perm[i]];

        auto f2 = [&](int dy, int dx) {
          f(dy, dx, 1);
          auto [y2, x2] = path[(int)path.size() - 2];
          if (y2 == y + dy && x2 == x + dx) homeFlagVec[d] = 1;
        };

        if (d == UP && !visited[y - 1][x]) f2(-1, 0);
        if (d == DOWN && !visited[y + 1][x]) f2(1, 0);
        if (d == LEFT && !visited[y][x - 1]) f2(0, -1);
        if (d == RIGHT && !visited[y][x + 1]) f2(0, 1);
      }
      for (int i = (int)homePath.size() - 1; i >= 0; i--)
        path.push_back(homePath[i]);
      // debug(path);
      // debug(homePath);

      makeVisitedVec(cells, path);
      ll nowScore = calcScore(cells, (int)path.size() - 1, 0);

      if (chmin(bestScore, nowScore)) bestPerm = perm;
      visited = nowVisited;
      path = nowPath;
      homePath = nowHomePath;
      ret = nowRet;
    } while (next_permutation(all(perm)));
    cells = nowCells;

    // debug(y, x, ret, bestPerm, explorable);
    // debug(visited);

    rep(i, explorable.size()) {
      int d = explorable[bestPerm[i]];

      auto f3 = [&](int dy, int dx) {
        if (homeFlagVec[d] == 1) homePath.emplace_back(y, x);
        f(dy, dx, 0);
        if (homeFlagVec[d] == 1) homePath.pop_back();
      };

      if (d == UP && !visited[y - 1][x]) f3(-1, 0);
      if (d == DOWN && !visited[y + 1][x]) f3(1, 0);
      if (d == LEFT && !visited[y][x - 1]) f3(0, -1);
      if (d == RIGHT && !visited[y][x + 1]) f3(0, 1);
    }
  }

  if (ret == -1 && (int)path.size() >= 3) {
    ...
    for (int i = (int)path.size() - 1; i >= 0; i--) {
      ...
      // 戻れるなら戻っていい
      // 直前に戻っても意味がないので直前は除外
      if (i <= (int)path.size() - 2 && movableSet.count(make_pair(y2, x2))) {
        returnable.push_back(i);
      }
    }
    if (returnable.size()) ret = returnable.back();
  }
  serachNow[y][x] = 0;
  return ret;
};
...
void dfsStart(Cells& cells, Path& path, vvi& visited, vvi& serachNow, int y,
  ...
  int playout = 0;
  Path homePath;
  dfs(cells, path, visited, serachNow, y, x, playout, homePath);
}
// 帰りにショートカットを入れている
