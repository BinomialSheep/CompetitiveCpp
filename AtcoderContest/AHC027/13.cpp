...
Path init4(Cells& cells, int dirtyN) {
  ...
  stk.emplace(0, 0);
  pushBackCell(cells, path, visited, 0, 0);
  visited[0][0] = false;
  int turn = 0;
  while (stk.size() || stk2.size()) {
    ...
    // 定期的に汚いところを掃除する
    if (turn % dirtyN == 1) {
      addDirtyPoints(stk2, visited);
    }
    // 取り出し
    int y = 0, x = 0;
    if (stk2.size()) {
      y = stk2.top().first, x = stk2.top().second;
      stk2.pop();
    } else {
      y = stk.top().first, x = stk.top().second;
      stk.pop();
    }
    if (visited[y][x]) continue;
    // 直前の頂点と繋げる
    if (!isAdjacent(y, x, path.back().first, path.back().second, cells)) {
    ...
    if ((path.back().first == y && path.back().second == x)) {
      visited[y][x] = true;
    } else {
