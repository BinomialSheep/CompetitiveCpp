...
Path init3(Cells& cells) {
  ...
  while (stk.size()) {
    ...
    if (explorable.size()) {
      ...
      // debug(y, x, ny, nx)
      // ny, nxまで経路復元
      vector dist(N, vi(N, 1e9));  // TODO：使いまわす
      dist[y][x] = 0;
      ...
      while (ny != y || nx != x) {
        ...
        if (cells[ny][nx].up && dist[ny - 1][nx] == dist[ny][nx] - 1) {
          ny--;
        } else if (cells[ny][nx].down && dist[ny + 1][nx] == dist[ny][nx] - 1) {
          ny++;
        } else if (cells[ny][nx].left && dist[ny][nx - 1] == dist[ny][nx] - 1) {
          nx--;
        } else if (cells[ny][nx].right &&
