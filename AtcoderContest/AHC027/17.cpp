...
Path init4(Cells& cells, int dirtyN) {
  ...
  while (++turn) {
    // 無限ループしてそうなので帰る
    if (turn >= 50000) {
      path[0] = make_pair(-1, -1);
      break;
    }
    if (dirtyN != -1 && turn % dirtyN == 0) {
      for (auto [ny, nx] : dirtyPoints) {
...
bool addNeighbor(Cells& cells, Path& path, ll& bestScore, double tempInv) {
  int i;
  int y, x;
  int direct;
  int flag = 0;
  ...
  ll deltaScore = bestScore - nowScore;
  // ダメなら戻す
...
bool delNeighbor(Cells& cells, Path& path, ll& bestScore, double tempInv) {
  int i;
  i = randXor() % (int)path.size();
  // 削除できるか判定
  auto [y, x] = path[i];
...
int main() {
  ...
  const static double START_TEMP = 1e5;  // 開始時の温度
  ...
  for (int dirtyN = 200; dirtyN <= 1000; dirtyN += 50) {
    initCells();
    path = init4(cells, dirtyN);
    if (path[0].first == -1) continue;
    string ans = path2str(path);
    ll score = calcScore(cells, (int)ans.size());
    // debug(dirtyN, score);
  ...
  while (++cnt) {
    ...
    // 追加近傍
    {
      bool isChange = addNeighbor(cells, path, bestScore, tempInv);
      if (isChange) addCnt++;
    }
    // // 削除近傍
    {
      bool isChange = delNeighbor(cells, path, bestScore, tempInv);
      if (isChange) delCnt++;
    }
    // 2opt近傍
    // {
