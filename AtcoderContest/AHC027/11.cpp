...
pair<int, int> calcBranchPoint(vvi& parent, Cells& cells, vvi& visited, int y,
                               int x) {
  int ny = y, nx = x;
  while (true) {
    if (parent[ny][nx] == 5) break;
    if (parent[ny][nx] == UP)
      ny--;
    else if (parent[ny][nx] == DOWN)
      ny++;
    else if (parent[ny][nx] == LEFT)
      nx--;
    else if (parent[ny][nx] == RIGHT)
      nx++;
    // まだ訪れていない場所があるならその辺まで戻る
    if (cells[ny][nx].up && !visited[ny - 1][nx]) break;
    if (cells[ny][nx].down && !visited[ny + 1][nx]) break;
    if (cells[ny][nx].left && !visited[ny][nx - 1]) break;
    if (cells[ny][nx].right && !visited[ny][nx + 1]) break;
  }
  return {ny, nx};
}
...
pair<int, int> connectPath(int y, int x, int ny, int nx, Cells& cells,
  ...
  debug(homePathStk, path.back(),
        visited[path.back().first][path.back().second]);
  return ret;
}
...
Path init3(Cells& cells) {
  ...
  stk.emplace(0, 0);
  vector parent(N, vi(N, 5));  // 行きがけに親を記録
  while (stk.size()) {
    // 取り出し
    auto [y, x] = stk.top();
    stk.pop();
    if (visited[y][x]) continue;
    // 直前の頂点と繋がっていないなら繋げる必要がある
    if (path.size() &&
      ...
      if (res.first != -1) {
        // 別の探索が始まる……
        stk.emplace(y, x);
        visited[res.first][res.second] = 0;
        stk.push(res);
        continue;
      }
    } else {
    ...
    if (cells[y][x].up && !visited[y - 1][x])
      parent[y - 1][x] = DOWN, explorable.push_back(UP);
    if (cells[y][x].down && !visited[y + 1][x])
      parent[y + 1][x] = UP, explorable.push_back(DOWN);
    if (cells[y][x].left && !visited[y][x - 1])
      parent[y][x - 1] = RIGHT, explorable.push_back(LEFT);
    if (cells[y][x].right && !visited[y][x + 1])
      parent[y][x + 1] = LEFT, explorable.push_back(RIGHT);
    ...
    if (explorable.size()) {
      ...
      // 分岐点まで戻る
      auto [ny, nx] = calcBranchPoint(parent, cells, visited, y, x);
      debug(y, x, ny, nx);
      if (isAdjacent(y, x, ny, nx, cells)) {
        continue;
      } else {
        // TODO：直接分岐点の隣接未訪問点に移動してもいいね。
        visited[ny][nx] = 0;
        stk.emplace(ny, nx);
        debug(y, x, ny, nx);
      }
