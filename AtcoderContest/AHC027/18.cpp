...
Path init4(Cells& cells, int dirtyN) {
  ...
  while (++turn) {
    ...
    if (dirtyN != -1 && turn % dirtyN == 0) {
      set<pair<int, int>> st;
      for (auto p : dirtyPoints) st.insert(p);
      ...
      while (st.size()) {
        ...
        while (q.size()) {
          ...
          q.pop();
          int now = dist[y2][x2];
          if (st.count(make_pair(y2, x2)) && (chmin(d, now) || now == d)) {
            ny = y2, nx = x2;
            break;
          }
          if (cells[y2][x2].up && chmin(dist[y2 - 1][x2], now + 1))
...
int main() {
  ...
  rep(i, min(30, (int)pq.size())) q.push(pq.top().second), pq.pop();
  while (q.size()) {
    auto p = q.front();
    q.pop();
  ...
  for (int dirtyN = 100; dirtyN <= 800; dirtyN += 20) {
    initCells();
  ...
  // debug(cnt, bestDirtyN, addCnt, delCnt);
