...
struct Solver {
  void solve() {
    ...
    /* solve */
    vvvi vec(H, vvi(W, vi(4)));
    // l, t, r, u
    rep(i, H) rep(j, W) {
      if (S[i][j] == '.') {
        vec[i][j][0] = ((j == 0) ? 0 : vec[i][j - 1][0]) + 1;
        vec[i][j][1] = ((i == 0) ? 0 : vec[i - 1][j][1]) + 1;
      }
    }
    for (int i = H - 1; i >= 0; i--)
      for (int j = W - 1; j >= 0; j--) {
        if (S[i][j] == '.') {
          vec[i][j][2] = ((j == W - 1) ? 0 : vec[i][j + 1][2]) + 1;
          vec[i][j][3] = ((i == H - 1) ? 0 : vec[i + 1][j][3]) + 1;
        }
      }
    debug(vec);
