...
struct Solver {
  void solve() {
    ...
    auto debugMint = [&](vector<mint>& A) {
      rep(i, A.size()) debug(i, A[i].val());
    };

    auto f = [&](int L2) -> mint {
      vector dp(N + 1, vector<mint>(M + 1));

      vector<mint> path(N + 1);
      path[1] = 1;
      if (N >= 2) path[2] = 1;
      for (int i = 3; i <= N; i++) path[i] = path[i - 1] * i;

      // debugMint(path);
      vector<mint> cycle(N + 1);
      for (int i = 2; i <= N; i++) cycle[i] = path[i - 1];
      // debugMint(cycle);
      ...
      rep(i, N + 1) rep(j, M + 1) {
        for (int k = 1; k <= L2; k++) {
          ...
          if (i + k <= N && j + k - 1 <= M)
            dp[i + k][j + k - 1] +=
                dp[i][j] * path[k] * nCkMod.COM(N - i - 1, k - 1);
          // サイクル
          if (i + k <= N && j + k <= M)
            dp[i + k][j + k] +=
                dp[i][j] * cycle[k] * nCkMod.COM(N - i - 1, k - 1);
        }
      }

      // rep(i, N + 1) debugMint(dp[i]);
