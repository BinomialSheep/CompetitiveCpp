...
struct Solver {
  void solve() {
    ...
    vvvi dist(N, vvi(N, vi(4, INFTY)));

    queue<pair<pair<int, int>, int>> q;
    rep(i, 4) dist[Ax][Ay][i] = 0, q.emplace(make_pair(Ax, Ay), i);
    vi dy = {-1, -1, 1, 1};
    vi dx = {-1, 1, -1, 1};
    while (q.size()) {
      auto p = q.front();
      ...
      rep(i, 4) {
        ...
        while (true) {
          ...
          if (S[ny][nx] == '#') break;
          if (dist[ny][nx][i] <= d) break;
          q.emplace(make_pair(ny, nx), i);
          dist[ny][nx][i] = d;
        }
      }
    }
    /* output */
    // rep(i, 10) rep(j, N) rep(k, N) rep(h, 2) {
    //   if (dist[j][k][h] == i) debug(i, j, k, h);
    // }
