...
struct Solver {
  void solve() {
    ...
    /* solve */
    vpll ps = calcPrimeFactorSimple(A);

    debug(ps);

    mint tmp = 1;
    for (auto [a, b] : ps) tmp *= (mint)b * B + 1;
    debug(tmp.val());

    ll mx = ps[0].second;
    for (auto [a, b] : ps) chmax(mx, b);
    mint tmp2 = (mint)mx * B + 1;

    out((tmp / tmp2 * B * (B + 1) / 2).val());

    // mint tmp = (mint)335 * 168 * 168 * 14028;
