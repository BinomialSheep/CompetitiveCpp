...
struct Solver {
  void solve() {
    ...
    for (int i = (N / 2 % 2 ? N / 2 + 1 : N / 2); i < N; i += 2) {
      map<ll, string> tmp;
      for (auto [k, s] : mpY2) {
        tmp[k + A[i]] = s + "T";
        tmp[k - A[i]] = s + "D";
    ...
    map<ll, string> mpX2;
    mpX2[0] = "";
    for (int i = (N / 2 % 2 ? N / 2 : N / 2 + 1); i < N; i += 2) {
      map<ll, string> tmp;
      for (auto [k, s] : mpX2) {
        tmp[k + A[i]] = s + "T";
        tmp[k - A[i]] = s + "D";
    ...
    int now = 0;
    string ans = "";
    rep(i, max(sy.size(), sx.size())) {
      if (i < sy.size()) {
        if (now == 0 && sy[i] == 'T') {
          ans.push_back('L');
          now = 1;
        } else if (now == 0 && sy[i] == 'D') {
          ans.push_back('R');
          now = 3;
        } else if (now == 2 && sy[i] == 'T') {
          ans.push_back('R');
          now = 1;
        } else {
          ans.push_back('L');
      ...
      if (i < sx.size()) {
        if (now == 1 && sx[i] == 'T') {
          ans.push_back('R');
          now = 0;
        } else if (now == 1 && sx[i] == 'D') {
          ans.push_back('L');
          now = 2;
        } else if (now == 3 && sx[i] == 'T') {
          ans.push_back('L');
          now = 0;
        } else {
          ans.push_back('R');
