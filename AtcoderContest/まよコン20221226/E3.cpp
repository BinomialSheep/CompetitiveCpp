...
struct Solver {
  void solve() {
    ...
    if (S1.size() > S3.size() || S2.size() > S3.size()) {
    ...
    do {
      ...
      if (!mp[S1[0]] || !mp[S2[0]] || !mp[S3[0]]) continue;

      string t1 = S1, t2 = S2, t3 = S3;
      rep(i, S1.size()) t1[i] = (char)('0' + mp[t1[i]]);
      rep(i, S2.size()) t2[i] = (char)('0' + mp[t2[i]]);
      rep(i, S3.size()) t3[i] = (char)('0' + mp[t3[i]]);
