...
struct Solver {
  void solve() {
    ...
    vi ans(N);
    int now = 1;
    auto dfs = [&](auto f, int v = 0, int d = 1, int p = -1) -> void {
      nows[v] = now;
      now++;
      for (auto u : G[v]) {
        if (u == p) continue;
    ...
    dfs(dfs);
    //
    int start = 0, mxDepth = 1;
    rep(i, N) {
      if (chmax(mxDepth, depth[i])) start = i;
    }
    now = 0;
    dfs(dfs, start);
    debug(depth);
    ...
    vpii ps;
    rep(i, N) ps.emplace_back(depth[i], i);
    sort(all(ps));

    rep(i, N) {
      int j = N - 1 - i;
      ans[nows[i]] = ps[j].second + 1;
    }
    out(ans);
