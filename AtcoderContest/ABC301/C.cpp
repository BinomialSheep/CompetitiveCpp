...
struct Solver {
  void solve() {
    ...
    auto f = [&]() {
      ...
      set<char> st;
      rep(i, atcoder.size()) st.insert(atcoder[i]);
      // debug(st);
      //
      map<char, int> mps;
      map<char, int> mpt;
      ...
      for (char c = 'a'; c <= 'z'; c++) {
        if (mps[c] == mpt[c]) continue;
        if (!st.count(c)) return false;
        if (mps[c] > mpt[c]) cnts += mps[c] - mpt[c];
        if (mps[c] < mpt[c]) cntt += mpt[c] - mps[c];
      }

      // debug(cnts, cntt);
      if (cnts > mpt['@']) return false;
      if (cntt > mps['@']) return false;
