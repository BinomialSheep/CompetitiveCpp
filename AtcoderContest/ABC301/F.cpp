...
struct Solver {
  void solve() {
    ...
    vector<vector<mint>> dp(S.size(), (vector<mint>(4)));
    ...
    rep(i, S.size()) {
      ...
      if (isupper(S[i])) {
        ...
        // 大文字
        dp[i][1] = 26 * dp[i - 1][0] + 25 * (dp[i - 1][1] + dp[i - 1][2]);
        dp[i][2] = dp[i - 1][1] + dp[i - 1][2];
        // 小文字
        dp[i][0] = 26 * (dp[i - 1][0] + dp[i - 1][1] + dp[i - 1][3]);
        dp[i][3] = 26 * (dp[i - 1][2]);
      } else {
        dp[i][0] = dp[i - 1][0] + dp[i - 1][1] + dp[i - 1][3];
        dp[i][3] = dp[i - 1][2];
      }
    }
    // rep(i, 5) rep(j, 4) debug(dp[i][j].val());

    mint ans = 0;
    rep(i, 4) ans += dp[S.size() - 1][i];
    out(ans.val());
