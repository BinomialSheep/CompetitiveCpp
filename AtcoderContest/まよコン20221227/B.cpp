...
struct Solver {
  void solve() {
    ...
    // 回転させる
    double at = (double)H / 12 + (double)M / (60 * 12);
    at *= 360;
    at = 360 - at;
    double bt = (double)M / 60;
    bt *= 360;
    bt = 360 - bt;

    // debug(at, bt);

    auto rotate = [&](double& x, double& y, double deg) {
      deg = deg * 3.141592653589793238462 / 180;
      double retx = x * cos(deg) - y * sin(deg);
      double rety = x * sin(deg) + y * cos(deg);
      x = retx;
      y = rety;
    };
