...
struct Solver {
  void solve() {
    ...
    /* solve */
    vector<set<int>> charList(26);
    rep(i, N) { charList[S[i] - 'a'].insert(i); }
    // debug(charList);

    auto isRange = [&](int l, int r, int i) -> bool {
      // l以上r以下
      auto it = charList[i].lower_bound(l);
      if (it == charList[i].end()) return false;
    ...
    while (Q--) {
      ...
      if (ty == 1) {
        ...
        cin >> c;
        // 消す
        charList[S[i] - 'a'].erase(i);
        // 加える
        charList[c - 'a'].insert(i);
        // 変える
        S[i] = c;
      } else {
        INT(l, r);
