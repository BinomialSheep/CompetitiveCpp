...
struct Solver {
  ...
  void solve() {
    ...
    ll currentScore = 1e18;  // TODO：持ち方変えたので一旦これで

    /* 焼きなまし法 */
    const static double START_TEMP = 1e5;  // 開始時の温度
    const static double END_TEMP = 1e3;    // 終了時の温度
    const static double END_TIME = 0.95;   // 終了時間（秒）
    ...
    do {
      ...
      if (neighType < 2) {
        ...
        int prev = tr[idx - 1];
        int next = tr[idx];
        ll oldScore = calcDist(prev, next, cd);
        ll newScore =
            calcDist(prev, stationId, cd) + calcDist(stationId, next, cd);
        // 解の更新
        ...
        ll oldScore =
            calcDist(prev, stationId, cd) + calcDist(stationId, next, cd);
        ll newScore = calcDist(prev, next, cd);
        // 解の更新
        ll deltaScore = oldScore - newScore;
        const double probability = exp((double)deltaScore / temp);
        if (probability >= rand01()) {
        ...
        int d = (int)(MAX_DELTA * (1.0 - time) + MIN_DELTA * time);
        ...
        int i2 = tr[to - 1];
        int i3 = tr[to];
        ll d0123 = calcDist(i0, i1, cd) + calcDist(i2, i3, cd);
        ll d0213 = calcDist(i0, i2, cd) + calcDist(i3, i1, cd);
