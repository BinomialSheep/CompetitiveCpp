...
ll annealing(double endTime, vi &tr, ll currentScore) {
  ...
  do {
    ...
    if (neighType < 5) {
      ...
      newP.first = mn + randXor() % (min(MAP_SIZE, newP.first + d) - mn);
      mn = max(0, newP.second - d);
      newP.second = mn + randXor() % (min(MAP_SIZE, newP.second + d) - mn);
...
int main() {
  ...
  rep(i, 3) {
    vi thisTr = initTr;
    ll thisScore = annealing(0.32, thisTr, currentScore);
