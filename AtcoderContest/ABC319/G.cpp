...
struct Solver {
  void solve() {
    ...
    set<int> nonvisited;
    rep(i, N) nonvisited.insert(i);
    queue<int> q;
    q.push(0);
    nonvisited.erase(0);
    vi dist(N, -1);
    dist[0] = 0;
    while (q.size() && nonvisited.size()) {
      auto v = q.front();
      q.pop();
      set<int> nonvisited2;
      for (auto u : nonvisited) {
        if (G[v].count(u)) {
          nonvisited2.insert(u);
        } else {
          dist[u] = dist[v] + 1;
          q.push(u);
        }
      }
      swap(nonvisited, nonvisited2);
    }
    ...
    int sz = -1;
    rep(i, N) mp[dist[i]].push_back(i), chmax(sz, dist[i]);
