...
struct Solver {
  void solve() {
    ...
    for (int i = 1; i <= M; i++) {
      ...
      ll idx = it - B.begin();
      // 最低でもidx-1をKまで持ってくる必要がある
      auto it2 = lower_bound(all(B), i);
      ll idx2 = it2 - B.begin();
      // 最大でもidx2-1がKに来たらダメ
      // i以下にk - idx+1個以上かつ、i未満にK - idx2+1以下な確率を求める
      // cnt回ガチャを引く
      rep(j, cnt + 1) {
        // j回Kが出る確率は、cntCj×(1/M)*(M-1/M)
        mint p = nCkMod.COM(cnt, j);
        p *= (mint(1) / mint(M)).pow(j);
        p *= (mint(M - 1) / mint(M)).pow(cnt - j);
        //
        rep(a, cnt + 1 - j) {
          // i以下にj+a回かつi未満にa回
          if (j + a < K - idx + 1) continue;
          if (a >= K - idx2 + 1) continue;
          // a回未満が出る確率
          mint p2 = nCkMod.COM(cnt - j, a);
          p2 *= (mint(i - 1) / mint(M - 1)).pow(a);
          p2 *= (mint(M - 1 - (i - 1)) / mint(M - 1)).pow(cnt - j - a);
          ans += i * p * p2;
        }
      }
    }
