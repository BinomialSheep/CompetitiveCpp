...
struct Solver {
  void solve() {
    ...
    /* solve */
    using mii = unordered_map<int, mint>;
    vector<vector<mii>> dp(H, vector<mii>(W));
    dp[0][0][0] = 1;
    int N = H + W;
    ...
    rep(i, H) rep(j, W) {
      ...
      if (i + 1 < H - 1) {
        mii mp;
        for (auto [k1, v1] : dp[i][j]) {
          for (auto [k2, v2] : dp[i + 1][j]) {
            if (S[i][j] == 'Y' && S[i + 1][j] == 'Y') {
              mp[k1 + k2 + 1] += v1 + v2;
            } else {
              mp[k1 + k2] += v1 + v2;
