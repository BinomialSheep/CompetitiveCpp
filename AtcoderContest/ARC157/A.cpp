...
struct Solver {
    ...
    void myTest(int N, int A, int B, int C, int D) {
    set<vi> st;
    int goal = N;

    auto f = [&](auto rec, string s) -> void {
      if(s.size() == goal) {
        vi v(4);
        rep(i, s.size()-1) {
          if(s[i] == 'X' && s[i+1] == 'X') v[0]++;
          if(s[i] == 'X' && s[i+1] == 'Y') v[1]++;
          if(s[i] == 'Y' && s[i+1] == 'X') v[2]++;
          if(s[i] == 'Y' && s[i+1] == 'Y') v[3]++;
        }
        st.insert(v);
        return;
      }
