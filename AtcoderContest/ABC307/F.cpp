...
struct Solver {
  void solve() {
    ...
    rep(i, K) {
      st.insert(A[i]);
      ans[A[i]] = 0;
      for (auto p : G[A[i]]) pq.emplace(p.second, p.first);
    }
    debug(pq);

    rep(i, D) {
      priority_queue<pii, vpii, greater<pii>> pq2;
      while (pq.size()) {
        auto p = pq.top();
        if (p.first > X[i]) break;
        pq.pop();
        if (st.count(p.second)) continue;
        st.insert(p.second);
        ans[p.second] = i + 1;
        for (auto pp : G[p.second])
          if (!st.count(pp.first)) {
            pq.emplace(p.first + pp.second, pp.first);
            pq2.emplace(pp.second, pp.first);
          }
      }
      while (pq2.size()) {
        pq.push(pq2.top());
