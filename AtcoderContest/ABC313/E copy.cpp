...
struct Solver {
  void solve() {
    ...
    deque<pair<char, mint>> dq;

    rep(i, N) {
      if (dq.empty() || dq.back().first != S[i]) {
        dq.push_back({S[i], 1});
      } else {
        dq.back().second += 1;
      }
    }
    // for (int i = 0; i < dq.size(); i++) debug(dq[i].first,
    // dq[i].second.val());
    ...
    while (dq.size() > 1) {
      ...
      dq.pop_back();
      if (p.first != '1') {
        dq.back().second += (ans + 1) * (p.first - '0') - ans;
      } else {
        ans += p.second;
      }
    }
    ...
    // ll ans = 0;
    // auto stupid = [&](auto f, string& s, int cnt = 0) {
    //   ans++;
    //   if (cnt >= 1000) {
    //     out(-1);
    //     return;
    ...
    //   string T;
    //   rep(i, s.size() - 1) {
    //     rep(j, s[i + 1] - '0') { T.push_back(s[i]); }
    //   }
    //   // debug(T);
    //   if (T.size() > 1) f(f, T, cnt + 1);
    // };
