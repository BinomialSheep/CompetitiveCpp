...
struct Solver {
  void solve() {
    ...
    /* solve */
    vvi vec(2 * N + 1, vi(2 * N + 1));
    rep(i, 2 * N) rep(j, 2 * N) {
      if (P[i % N][j % N] == 'B') vec[i][j] += 1;
      vec[i][j + 1] += vec[i][j];
    }
    rep(i, 2 * N) rep(j, 2 * N) { vec[i + 1][j] += vec[i][j]; }
    ...
    rep(qi, Q) {
      ...
      if (ty && tx) {
        ans += (ty * tx) * vec[N - 1][N - 1];
      } else if (ty) {
        ans += ty * (vec[N - 1][D] - (B > 0 ? vec[N - 1][B - 1] : 0));
      } else if (tx) {
        ans += tx * (vec[B][N - 1] - (A > 0 ? vec[A - 1][N - 1] : 0));
      }
      debug(ans);
