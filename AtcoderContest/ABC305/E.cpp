...
struct Solver {
  void solve() {
    ...
    dsu uf(N);
    rep(i, N) for (auto u : G[i]) uf.merge(i, u);
    ...
    vi vec2(N, -1);
    vi visited(N, -1);
    rep(i, N) if (vec[i] >= 0) vec2[uf.leader(i)]++;
    ...
    while (!pq.empty() && cnt < N) {
      ...
      visited[v] = d;
      if (vec2[uf.leader(v)] == uf.size(v)) continue;
      for (auto u : G[v]) {
        if (vec[u] == -1) cnt++;
        if (chmax(vec[u], d - 1) && d - 1 > 0) {
          pq.emplace(d - 1, u);
        }
      }
      // debug(vec);
    ...
    out(cnt);
    rep(i, N) if (vec[i] >= 0) out(i + 1);
    // printf("%d\n", cnt);

    // rep(i, N) {
    //   if (vec[i] >= 0) printf("%d ", i + 1);
    // }
