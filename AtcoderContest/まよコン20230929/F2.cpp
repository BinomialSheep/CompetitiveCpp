  ...
  void solve() {
    ...
    rep(i, N) {
      ...
      if (i == 0) {
        ...
        dp2[0][0] = max(dp[0][0], dp[1][0] + B[i - 1]);
        dp2[0][1] = max(dp[0][1], dp[1][1] + B[i - 1]);
        dp2[1][0] = max(dp[1][0], dp[0][0] + B[i - 1]) + A[i];
        dp2[1][1] = max(dp[1][1], dp[0][1] + B[i - 1]) + A[i];
      }
      swap(dp, dp2);
    }
    ll ans = 0;
    chmax(ans, dp[0][0]);
    chmax(ans, dp[0][1] + B[N - 1]);
    chmax(ans, dp[1][0] + B[N - 1]);
    chmax(ans, dp[1][1]);
    out(sum - ans);
