...
struct Solver {
  void solve() {
    ...
    rep(i, N) E.emplace_back(A[i], make_pair(0, i + 1));
    rep(i, N - 1) E.emplace_back(B[i], make_pair(i + 1, i + 2));
    E.emplace_back(B[N - 1], make_pair(1, N));
    sort(all(E));
    reverse(all(E));
    // debug(E);
