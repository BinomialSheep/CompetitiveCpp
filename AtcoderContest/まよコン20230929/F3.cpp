  ...
  void solve() {
    ...
    rep(i, N) {
      ...
      if (i == 0) {
        ...
        dp2[0][0] = min(dp[0][0] + B[i - 1], dp[1][0]) + A[i];
        dp2[0][1] = min(dp[0][1] + B[i - 1], dp[1][1]) + A[i];
        dp2[1][0] = min(dp[1][0] + B[i - 1], dp[0][0]);
        dp2[1][1] = min(dp[1][1] + B[i - 1], dp[0][1]);
      }
      swap(dp, dp2);
      debug(dp);
    }
    ll ans = 1e18;
    chmin(ans, dp[0][0] + B[N - 1]);
    chmin(ans, dp[0][1]);
    chmin(ans, dp[1][0]);
    chmin(ans, dp[1][1] + B[N - 1]);
    out(ans);
