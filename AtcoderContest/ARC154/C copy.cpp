...
struct Solver {
  void solve() {
    ...
    rep(ti, T) {
      ...
      rep(i, N) mp[A[i]].push_back(i + N);

      debug(mp);

      int isNo = 0;
      rep(i, N) {
        if (mp.count(B[i]) == 0) isNo = 1;
      }
      if (isNo) {
        out("No");
        continue;
      ...
      rep(j, N) {
        ...
        rep(i, 2 * N) {
          ...
          chmax(now, i);
          if (mp2.count(B[id])) {
            graph.add_edge(mp2[B[id]], id);
            continue;
          }

          auto it = lower_bound(all(mp[B[id]]), now);
          if (it == mp[B[id]].end()) {
            isNo = 1;
            break;
          }
          now = *it;
          graph.add_edge(i, (*it) % N);
          if (!mp2.count(B[id])) mp2[B[id]] = id;
        }
