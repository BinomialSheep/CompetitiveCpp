...
int calcSize2(vvi &t) {
  int ret = 0;
  vvvi scorelist = calcSizeList(t);
  rep(i, N) rep(j, N) { chmax(ret, scorelist[i][j][4]); }
  return ret;
}
...
int main() {
  ...
  int maxIndex = -1;
  vvi originT = t;
  vi originBlank = blank;

  static double START_TEMP = 1500;     // 開始時の温度
  const static double END_TEMP = 100;  // 終了時の温度
  const static double END_TIME = 2.9;  // 終了時間（秒）
  double time = 0.0;                   // 経過時間（秒）

  do {
    // 進捗。開始時が0.0で、終了時が1.0（ループによって変わる）
    ...
    if (blank[0] != 0) {
      ...
      changeScoreList.pushback(calcSize2(t));
      swap(t[blank[0]][blank[1]], t[blank[0] - 1][blank[1]]);
    }
    if (blank[1] != N - 1) {
      swap(t[blank[0]][blank[1]], t[blank[0]][blank[1] + 1]);
      changeScoreList.push_back(calcSize2(t));
      swap(t[blank[0]][blank[1]], t[blank[0]][blank[1] + 1]);
    }
    if (blank[0] != N - 1) {
      swap(t[blank[0]][blank[1]], t[blank[0] + 1][blank[1]]);
    ...
    if (probability >= rand01()) {
      ...
      if ((int)ans.size() == T) {
        if (maxSize > bestSize) {
          // 今回のループを採用
          bestSize = maxSize;
          bestAns.resize(maxIndex);
          rep(i, maxIndex) bestAns[i] = ans[i];
          if (maxSize == N * N - 1) break;
        }
        // もろもろリセット
        maxSize = -1;
        maxIndex = -1;
  ...
  /* output */
  // cout << time << endl;
  rep(i, (int)bestAns.size()) cout << bestAns[i];
  cout << endl;

  // cout << round(500000 * bestSize / (N * N - 1)) << endl;
