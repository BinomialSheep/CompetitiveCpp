...
struct Solver {
  void solve() {
    ...
    /* solve */
    vi B;
    rep(i, N) rep(j, M) B.push_back(A[i][j]);
    sort(all(B));
    unordered_map<int, int> inv;
    rep(i, B.size()) inv[B[i]] = i;
    ...
    // 単位元 S e()
    auto e = []() -> S { return 0LL; };
    segtree<S, op, e> seg((int)B.size());  // A(N, e)で初期化
    auto op2 = [](S a, S b) -> S { return a + b; };
    // 単位元 S e()
    segtree<S, op2, e> seg2((int)B.size());  // A(N, e)で初期化

    ll ans = 0;
    rep(i, N) {
      int left = 0;
      rep(j, M) {
        // debug(left, inv[A[i][j]]);
        // debug(left, inv[A[i][j]]);
        ans += seg.prod(left, inv[A[i][j]]);
        ans += seg2.prod(left, inv[A[i][j]]) * j;
        left = inv[A[i][j]];
        // debug(i, j, ans);
      }
      // debug(left, (int)B.size());
      ans += seg.prod(left, (int)B.size());
      ans += seg2.prod(left, (int)B.size()) * M;

      rep(j, M) seg.set(inv[A[i][j]], j + 1);
      rep(j, M) seg2.set(inv[A[i][j]], 1);
      // debug(ans);
    }
