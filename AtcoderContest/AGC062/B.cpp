...
struct Solver {
  vector<int> lis(int n, vi l) {
    int INF = 1e9 + 10;
    vector<int> lisDP(n, INF);
    vector<int> indexList(n, -1);
    rep(i, n) {
      int ind = (int)distance(lisDP.begin(),
                              lower_bound(lisDP.begin(), lisDP.end(), l.at(i)));
      lisDP.at(ind) = l[i];
      indexList.at(i) = ind;
    }
  ...
  void solve() {
    ...
    auto f = [&](auto rec, ll now, int depth, vi vec) {
      if (now > num) return;
      if (depth == K) {
        if (now == num) debug(vec);
        // vec: [ 0 0 0 10 0 0 1 2 7 8 ]
        return;
      }
      rep(i, N) {
        vec.push_back(i);
        rec(rec, now + C[depth] * i, depth + 1, vec);
        vec.pop_back();
      }
    };
    vi tmp;
