...
struct Solver {
  void solve() {
    ...
    rep(i, N - 1) {
      ...
      --A;
      --B;
      for (auto v : mp[lead[A]]) {
        ans[v] += (mint)mp[lead[A]].size() /
                  (mint)((int)mp[lead[A]].size() + (int)mp[lead[B]].size());
      }
      for (auto v : mp[lead[B]]) {
        ans[v] += (mint)mp[lead[B]].size() /
                  (mint)((int)mp[lead[A]].size() + (int)mp[lead[B]].size());
      }

      if (mp[lead[A]].size() <= mp[lead[B]].size()) {
        for (auto v : mp[lead[A]]) {
          lead[v] = lead[B];
          mp[lead[B]].insert(v);
        }
