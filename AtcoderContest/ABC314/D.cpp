...
struct Solver {
  void solve() {
    ...
    vi t(Q), x(Q);
    vector<char> c(Q);
    rep(qi, Q) cin >> t[qi] >> x[qi] >> c[qi];

    int ty = -1;
    int num = -1;
    rep(qi, Q) {
      if (t[qi] == 2) ty = 2, num = qi;
      if (t[qi] == 3) ty = 3, num = qi;
    }
    ...
    rep(qi, Q) {
      if (t[qi] == 1) {
        S[x[qi] - 1] = c[qi];
      } else if (t[qi] == 2 && qi == num) {
        transform(all(S), S.begin(),
                  [](char const& ch) { return tolower(ch); });
      } else if (t[qi] == 3 && qi == num) {
        transform(all(S), S.begin(),
                  [](char const& ch) { return toupper(ch); });
      }
    }
    out(S);
