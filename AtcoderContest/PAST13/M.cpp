...
struct Solver {
  void solve() {
    ...
    rep(i, N - 1) {
      INT(u, v);
      --u, --v;
      G[u].emplace_back(i + 1, v);
      G[v].emplace_back(i + 1, u);
    }
    // debug(G);

    /* solve */
    ll ans = 0;
    // auto g = [N](int l, int r) -> ll { return ll(l - r) * (l - r + 1) / 2; };
    auto g = [N](ll l, ll r) -> ll { return ll(l) * (N - r); };
    auto dfs = [&](auto f, ll v, ll l, ll r, ll p = -1) -> void {
      for (auto [k, u] : G[v]) {
        if (u == p) continue;
        // debug(v, u);
        // debug(k, u);
