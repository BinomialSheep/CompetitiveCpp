...
struct Solver {
  void solve() {
    ...
    rep(i, N) cin >> B[i];
    {
      int f = 0, f3 = 0;
      rep(i, N - 2) {
        int a = A[i] + A[i + 1] + A[i + 2];
        int b = B[i] + B[i + 1] + B[i + 2];
        if (a % 2 == 0) {
          f = 1;
        }
        if (b % 2 == 0) {
    ...
    auto f = [&] {
      int sz = A.size();
      if ((A[0] + A[1] + A[2]) % 2 == 0) return true;
      if ((A[sz - 3] + A[sz - 2] + A[sz - 1]) % 2 == 0) return true;
      return false;
    };
    ...
    /* solve */
    while (A.size() > 3 && A.front() == B.front()) {
      A.pop_front();
      B.pop_front();
      if (f()) {
        out("Yes");
    ...
    assert(false);
    bool isYes = true;
    rep(i, A.size()) if (A[i] != B[i]) isYes = false;
    Yes(isYes);
