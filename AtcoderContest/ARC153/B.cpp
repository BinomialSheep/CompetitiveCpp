...
struct Solver {
  void solve() {
    ...
    int yoko = 0;
    INT(Q);
    for (int qi = 0; qi + 1 < Q; qi += 2) {
      INT(a1, b1, a2, b2);
      if (a1 < a2) {
        rep(i, a2 - a1) {
          tate.push_front(tate.back());
          tate.pop_back();
        }

      } else if (a1 > a2) {
        rep(i, a1 - a2) {
          tate.push_back(tate.front());
          // st.push(tate.front());
          tate.pop_front();
        }
      }
      if (b1 < b2) {
        rep(i, b2 - b1) {
          // st.push(yoko.back());
          yoko.push_front(yoko.back());
          yoko.pop_back();
        }
      } else if (b1 > b2) {
        rep(i, b1 - b2) {
          // st.push(yoko.front());
          yoko.push_back(yoko.front());
          yoko.pop_front();
        }
      }
    }
    ...
    vs ans = A;
    rep(i, H) {
      rep(j, W) { ans[i][j] = A[tate[i]][yoko[j]]; }
    }
    ...
    if (Q % 2) {
      ...
      reverse(ans.begin(), ans.begin() + a);
      // debug(ans);
      reverse(ans.begin() + a, ans.end());
      rep(i, H) {
        reverse(ans[i].begin(), ans[i].begin() + b);
        reverse(ans[i].begin() + b, ans[i].end());
      }
    }
