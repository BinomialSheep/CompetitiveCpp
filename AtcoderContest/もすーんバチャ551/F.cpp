...
class myCls {
 public:
  ...
  ll value;
  myCls() {}
  myCls(int y_, int x_, ll v) : y(y_), x(x_), value(v) {}
  // bool operator<(const myCls& c) { return value < c.value; };
  // bool operator>(const myCls& c) { return value > c.value; };
};

bool operator<(const myCls& c1, const myCls& c2) { return c1.value > c2.value; }
...
struct Solver {
  void solve() {
    ...
    rep(i, K - 1) {
      ...
      if (y + 1 < N && !used[y + 1][x]) {
        used[y + 1][x] = true;
        pq.emplace(y + 1, x, a[y + 1] * b[x]);
      }
      if (x + 1 < N && !used[y][x + 1]) {
        used[y][x + 1] = true;
        pq.emplace(y, x + 1, a[y] * b[x + 1]);
      }
    }
