...
struct Solver {
  void solve() {
    ...
    {
      set<int> vec;
      rep(i, H * W) vec.insert(i);
      st.insert(vec);
    }

    auto rec = [&](auto f, set<set<int>> nowst, int t) -> ll {
      if (t == 0) {
        ll mn = 1e18;
        ll mx = 0;
        for (auto vec : nowst) {
      ...
      for (auto st2 : nowst) {
        ...
        set<set<int>> nxt = nowst;
        nxt.erase(st2);
        for (auto v : st2) {
          if (st2.count(v + 1) && !st2.count(v - W)) {
            set<int> st3;
            set<int> st4;
            for (auto v2 : st2) {
              if (v2 % W <= v % W)
            ...
            nxt.insert(st3);
            nxt.insert(st4);
            chmin(ret, f(f, nxt, t - 1));
            nxt.erase(st3);
            nxt.erase(st4);
          }
          if (st2.count(v + W) && !st2.count(v - 1)) {
            set<int> st3;
            set<int> st4;
            for (auto v2 : st2) {
              if (v2 / W <= v / W)
