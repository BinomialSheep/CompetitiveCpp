...
struct Solver {
  void solve() {
    /* input */
    INT(H, W, M);
    vector<set<int>> setH(H);
    vector<set<int>> setW(W);
    rep(i, M) {
      INT(h, w);
      --h, --w;
      setH[h].insert(w);
    ...
    int mxh = 0;
    rep(i, H) chmax(mxh, (int)setH[i].size());
    vi hs;
    rep(i, H) if (setH[i].size() == mxh) hs.push_back(i);
    int mxw = 0;
    rep(i, W) chmax(mxw, (int)setW[i].size());
    vi ws;
    rep(i, W) if (setW[i].size() == mxw) ws.push_back(i);
