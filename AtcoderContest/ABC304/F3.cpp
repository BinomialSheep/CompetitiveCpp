...
// シンプルな約数列挙(O(√N))
// 1e6以下の最大の約数の個数は240、1e9で1344,
// 1e12で6720, 1e18で103680
...
struct Solver {
  void solve() {
    ...
    for (int i = N - 1; i >= 1; i--) {
      ...
      vec.push_back(i);
      ans += mp[i];
      vl divs = calc_divisors(i);
      for (auto v : divs) color[v] = 1;
    }
    // debug(vec);
    ...
    rep(i, vec.size()) rep(j, i) {  //
      mp2[mp[gcd(vec[j], vec[i])].val()]++;
      // ans -= mp[gcd(vec[j], vec[i])];
    }
    for (auto [k, v] : mp2) {
      int a = -1;
      for (int i = 1; i <= v; i++) {
