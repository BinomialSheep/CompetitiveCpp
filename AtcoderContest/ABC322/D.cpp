    ...
    rep(j, 4) rep(k, 4) {
      ...
      for (int iy = -3; iy < 4; iy++)
        for (int ix = -3; ix < 4; ix++)
          for (int jy = -3; jy < 4; jy++)
            for (int jx = -3; jx < 4; jx++)
              for (int ky = -3; ky < 4; ky++)
                for (int kx = -3; kx < 4; kx++) {
                  map<pair<int, int>, int> mp;
                  rep(y, 4) rep(x, 4) {
                    if (p1[y][x] == '#') mp[make_pair(y + iy, x + ix)]++;
                    if (p2[y][x] == '#') mp[make_pair(y + jy, x + jx)]++;
                    if (p3[y][x] == '#') mp[make_pair(y + ky, x + kx)]++;
                  }
                  int flag = 1;
                  vvi A(4, vi(4));
                  for (auto [p, v] : mp) {
                  ...
                  if (flag == 0) continue;
                  rep(i2, 4) rep(j2, 4) if (A[i2][j2] != 1) flag = 0;
                  if (flag) {
                    out("Yes");
                    return;
                  }
