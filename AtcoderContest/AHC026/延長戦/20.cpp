...
void solve(vvi& B, vi& C) {
  ...
  auto isPuttableAdjacent = [&](int i, int j, int i2,
                                unordered_map<int, int>& adjacentMp) {
    if (i == i2) return false;
  ...
  rep(ci, M) {
    ...
    while (B[i].size()) {
      ...
      vpii priority;
      rep(i2, M) {
        if (isPuttableAdjacent(i, k, i2, adjacentMp)) {
          // 優先度最高
    ...
    while (sorted.size() && sorted.back() >= now) {
      // if (B[1].size() >= 10 && B[1][11] == 26 && sorted.back() == 25)
      //   // debug(B, sorted, poppedSet, adjacentMp, leaderMp);
      // if (B[1].size() >= 15 && B[1][13] == 16 && B[1][14] == 17)
      //   // debug(B, sorted, poppedSet), assert(false);
              ...
              rep(i3, M) {
                ...
                if ((sorted.empty() || B[i3].back() > sorted.back()) &&
                  ...
                  while (k != 0 && B[i3][k - 1] > B[i3][k] &&
                    ...
                    k--;
                  // if (B[i3][k] == 16) debug(B, sorted, poppedSet);
                  operation1(i3, k, i);
                  topCheck();
                  flag = 1;
                  break;
