...
struct Solver {
  void solve() {
    ...
    auto sol = [&](vi& A, vi& C) {
      ...
      for (int ci = 1; ci < M; ci++) {
        ...
        while (B[i].size()) {
          topCheck(ans, now);
          if (B[i].empty()) break;
          // 一番上を移動できるならもう移動させてしまう
          int flag = 0;
          rep(k, A.size()) {
            if (B[i].back() <= A[k]) {
              for (int ci2 = 1; ci2 < ci; ci2++) {
                if (C[k] == C[ci2]) {
                  flag = 1;
                  break;
                }
              ...
              while (j >= 1 && B[i][j] < B[i][j - 1] &&
                     A[k - 1] < B[i][j - 1] && B[i][j - 1] <= A[k])
                j--;
              ans.emplace_back(B[i][j], C[k] + 1);
              V += (int)B[i].size() + 1 - j;
              operation1(i, j, C[k]);
              // debug(i, j, B[i][j], C[k], B);
              break;
            }
          }
          if (flag) continue;
    ...
    // デバッグ用
    // {
    //   vi A = {0, 22, 44, 66, 88, 110, 132, 154, 176, 200};
