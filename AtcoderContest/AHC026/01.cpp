...
struct Solver {
  void solve() {
    ...
    vi A = {0, 22, 44, 66, 88, 110, 132, 154, 177, 200};
    // 最初は0列目に入れる
    for (int i = 1; i < M; i++) {
      V += (int)B[i].size() + 1;
      operation1(i, 0, 0);
      ans.emplace_back(B[i][0], 1);
    }
    debug(V, B);
    // 上から順に処理
    while (B[0].size()) {
      ...
      rep(i, A.size()) {
        if (B[0].back() <= A[i]) {
          ans.emplace_back(B[0].back(), i + 1);
          operation1(0, (int)B[0].size() - 1, i);
          break;
        }
      }
    }
        ...
        for (int i2 = 0; i2 < M; i2++) {
          ...
          if (i2 == M - 1 || (i == M - 1 && i2 == M - 2)) {
            ...
            for (; j < (int)B[i].size(); j++) {
              if (B[i][j] == now) {
                int tmp = (int)B[0].size();
                operation1(i, j + 1, 0);
                ans.emplace_back(B[i][j + 1], 1);
                ans.emplace_back(now, 0);
                now++;
                operation2(i);
                ans.emplace_back(B[0][tmp], i + 1);
                operation1(0, tmp, i);
                break;
              }
            }
