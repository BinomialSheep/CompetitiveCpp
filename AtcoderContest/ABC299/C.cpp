...
struct Solver {
  void solve() {
    ...
    rep(i, N) {
      if (i == 0 || S[i] != runlength.back().first) {
        runlength.emplace_back(S[i], 1);
      } else {
    ...
    debug(runlength);
    int ans = -1;
    rep(i, runlength.size()) {
      if (runlength[i].first == 'o') continue;
      if (i != 0) chmax(ans, runlength[i - 1].second);
      if (i != runlength.size() - 1) chmax(ans, runlength[i + 1].second);
    }
    out(ans);
