...
struct Solver {
  void solve() {
    ...
    vi backs;
    vi fronts;
    rep(i, M) fronts.push_back(C[i][0]);
    rep(i, M) backs.push_back(C[i].back());
    ...
    rep(bi, M) {
      right = backs.back();
      debug(left, right);
      // debug(seg.prod(0, right + 1));
      B[bi] = seg.prod(left, right + 1);
      used.insert(B[bi]);
      debug(B[bi]);
      for (auto v : C[B[bi] - 1]) seg.set(v, INF);  // 選択済み
      while (left < N && A[left] != B[bi]) {
        seg.set(left, INF);
        left++;
      }
      left++;
      // for (int i = left; i <= fronts[B[bi] - 1]; i++) seg.set(i, INF);
      while (backs.size() &&
             (backs.back() < left || used.count(A[backs.back()])))
        backs.pop_back();
    }
    out(B);
