...
struct Solver {
  void solve() {
    ...
    rep(i, N) {
      ...
      int dist = (int)(distance(h.begin(), it));
      // debug(dist);
      auto it2 = upper_bound(all(w), w[i]);
      if (it2 == w.end()) continue;
      int dist2 = (int)(distance(w.begin(), it2));
      // debug(dist2);
      // if (dist > dist2) continue;
      auto it3 = upper_bound(all(d), d[i]);
      if (it3 == d.end()) continue;
      int dist3 = (int)(distance(d.begin(), it3));
      // debug(dist3);
      // if (dist2 > dist3) continue;
      ans = 1;
    }
    /* output */
    Yes(ans);
