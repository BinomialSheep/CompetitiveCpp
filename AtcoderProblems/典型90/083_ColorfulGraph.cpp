...
struct Solver {
  void solve() {
    ...
    int sq = sqrt(2 * M);
    set<int> big;
    rep(i, N) if ((int)G[i].size() >= sq) big.insert(i);
    vvi G2(N);
    rep(i, N) for (auto v : G[i]) {
      if (big.count(v)) G2[i].push_back(v);
    }
    vi color(N, 1);
    vi colored(N, -1);
    vi selected(N, -1);
    ...
    rep(qi, Q) {
      ...
      --x;
      // 今の色を答える
      int c = color[x], mx = colored[x];
      // debug(c, mx, G2[x]);
      // debug(x, y, c, mx);
      for (auto v : G2[x])
        if (chmax(mx, selected[v])) c = color2[v];
      out(c);
      if (big.count(x)) {
        // なにもしない
      } else {
        // 色を更新する
        for (auto v : G[x]) colored[v] = qi, color[v] = y;
      }
      selected[x] = qi, colored[x] = qi, color[x] = y, color2[x] = y;
      // debug(x, y);
      // debug(color);
      // debug(colored);
      // debug(selected);
