...
struct Solver {
  void solve() {
    ...
    vi dy = {-1, 1, 0, 0};
    vi dx = {0, 0, -1, 1};
    auto f = [&](auto rec, int sy, int sx, int y, int x, vvl& dist) -> ll {
      ll ret = -1;
      rep(i, 4) {
        int ny = y + dy[i];
        int nx = x + dx[i];
        if (ny == sy && nx == sx) {
          // debug(ny, nx, dist[y][x]);
          if (dist[y][x] >= 3) chmax(ret, dist[y][x] + 1);
          continue;
        }
        if (ny < 0 || ny >= H || nx < 0 || nx >= W || dist[ny][nx] ||
            c[ny][nx] == '#')
          continue;
        dist[ny][nx] = dist[y][x] + 1;
        // debug(sy, sx, ny, nx, dist[ny][nx]);
        chmax(ret, rec(rec, sy, sx, ny, nx, dist));
        dist[ny][nx] = 0;
      }
      // debug(y, x, ret);
      return ret;
