...
struct Solver {
  void solve() {
    ...
    // ダブリングする
    // i番目から2^k個進んだ地点
    vvi doubling(logK, vi(100000, -1));
    rep(i, 100000) doubling[0][i] = f(i);
    rep(k, logK - 1) {
      //
      rep(i, 100000) doubling[k + 1][i] = doubling[k][doubling[k][i]];
    }
    int ans = N;
    int now = 60;
    for (ll M = (1LL << 60); M > 0; M /= 2, now--) {
      if (M <= K) {
        debug(M, K, ans, now);
        K -= M;
        ans = doubling[now][ans];
      }
    }
    out(ans);
