...
struct Solver {
  void solve() {
    ...
    vi oidx, xidx;
    rep(i, N) {
      if (S[i] == 'o') oidx.push_back(i);
      if (S[i] == 'x') xidx.push_back(i);
    }
    debug(oidx, xidx);
    ...
    rep(i, N) {
      if (S[i] == 'o') {
        auto it = lower_bound(all(xidx), i);
        if (it != xidx.end()) ans += N - *it;
      } else {
        auto it = lower_bound(all(oidx), i);
        if (it != oidx.end()) ans += N - *it;
      }
      debug(ans);
    }
    out(ans);
