...
struct Solver {
  void solve() {
    ...
    /* solve */
    // 基数変換
    auto f = [](string s, int from, int radix) -> string {
      string ret;
      debug(s);
      ll m = 0;
      ll tmp = 1;
      rep(i, s.size()) {
        m += (s[s.size() - 1 - i] - '0') * tmp;
        tmp *= from;
      }
      if (m == 0) return "0";
      while (m) {
        debug(m, m % radix);
        ret.push_back('0' + (m % (ll)radix));
        m /= radix;
      }
      reverse(ret.begin(), ret.end());
      return to_string(stoll(ret));  // 先頭の0削除
    };
    rep(i, K) {
      // Nを9進数に直す
      string N9 = f(N, 8, 9);
      // 書き直す
      rep(j, N9.size()) if (N9[j] == '8') N9[j] = '5';
      // 8進数に直す
      N = N9;
      // N = f(N9, 9, 8);
    }
