...
struct Solver {
  void solve() {
    ...
    vl left(N);
    vl right(N);
    ll ls = 0, rs = 0, ts = 0, bs = 0;
    rep(i, N) {
      if (i == 0) continue;
      left[i] = ls += ll(i) * abs(X[i] - X[i - 1]);
    }
    for (int i = N - 1; i >= 0; i--) {
      if (i == N - 1) continue;
      right[i] = rs += ll(N - 1 - i) * abs(X[i] - X[i + 1]);
    }
    debug(left, right);
    ll horizon = 1e18;
    rep(i, N) chmin(horizon, left[i] + right[i]);

    vl top(N);
    vl bottom(N);
    rep(i, N) {
      if (i == 0) continue;
      top[i] = ts += ll(i) * abs(Y[i] - Y[i - 1]);
    }
    for (int i = N - 1; i >= 0; i--) {
      if (i == N - 1) continue;
      bottom[i] = bs += ll(N - 1 - i) * abs(Y[i] - Y[i + 1]);
    }
    debug(top, bottom);
    ll vertical = 1e18;
    rep(i, N) chmin(vertical, top[i] + bottom[i]);

    /* output */
    out(horizon + vertical);
  }
};
