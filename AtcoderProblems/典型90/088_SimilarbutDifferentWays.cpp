...
struct Solver {
  void solve() {
    ...
    vi vec;  // 現在の選択列
    auto dfs = [&](auto rec, int pos, int sum) -> void {
      if (flag) return;
      if (pos == N) {
      ...
      if (cnt[pos] == 0) {
        // 選ぶ
        vec.push_back(pos);
        for (auto v : G[pos]) cnt[v]++;
        rec(rec, pos + 1, sum + A[pos]);
        for (auto v : G[pos]) cnt[v]--;
        vec.pop_back();
      }
    };
    ...
    out(ans[ansidx][0].size());
    for (auto v : ans[ansidx][0]) cout << v + 1 << " ";
    out("");
    out(ans[ansidx][1].size());
    for (auto v : ans[ansidx][1]) cout << v + 1 << " ";
    out("");
