...
struct Solver {
  void solve() {
    ...
    auto dfs = [&](auto rec, int v, int p = -1) -> void {
      mint tmp1 = 1, tmp2 = 1;
      for (auto u : G[v]) {
        if (u == p) continue;
        rec(rec, u, v);

        if (C[v] == 'a') tmp1 *= dp[u][0] + dp[u][2];
        if (C[v] == 'b') tmp1 *= dp[u][1] + dp[u][2];
        tmp2 *= dp[u][0] + dp[u][1] + dp[u][2] * 2;
      }
      if (C[v] == 'a') dp[v][0] = tmp1;
      if (C[v] == 'b') dp[v][1] = tmp1;
      dp[v][2] = tmp2 - tmp1;
    };

    dfs(dfs, 0);
    // rep(i, N) rep(j, 3) debug(j, dp[i][j].val());
    out(dp[0][2].val());
