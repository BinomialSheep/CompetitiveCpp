...
struct Solver {
  void solve() {
    /* input */
    INT(N, K);
    vvi cnt(5001, vi(5001));
    rep(i, N) {
      INT(A, B);
      cnt[A][B]++;
    }
    rep(i, 5001) rep(j, 5001) if (j > 0) cnt[i][j] += cnt[i][j - 1];
    rep(i, 5001) rep(j, 5001) if (i > 0) cnt[i][j] += cnt[i - 1][j];
    // debug(cnt);
    // rep(i, 10) rep(j, 10) debug(i, j, cnt[i][j]);
    ...
    rep(i, 5001) rep(j, 5001) {
      ll now = cnt[i][j];
      if (j - K >= 0) now -= cnt[i][j - K];
      if (i - K >= 0) now -= cnt[i - K][j];
      if (j - K >= 0 && i - K >= 0) now += cnt[i - K][j - K];
      chmax(ans, now);
    }
    out(ans);
