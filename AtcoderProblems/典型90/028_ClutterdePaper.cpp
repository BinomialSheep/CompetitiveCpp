...
struct Solver {
  void solve() {
    ...
    rep(i, N) {
      ...
      imos[mxY + 1][mxX + 1]++;
      // rep(j, 10) {
      //   rep(k, 10) cout << imos[j][k] << ' ';
      //   cout << endl;
      // }
    }
    // debug(imos);

    rep(i, 1001) rep(j, 1000) imos[i][j + 1] += imos[i][j];
    rep(i, 1000) rep(j, 1001) imos[i + 1][j] += imos[i][j];
    ...
    vi ans(N + 1);
    rep(i, 1000) rep(j, 1000) ans[imos[i][j]]++;
    debug(ans);
    //
    rep(i, N) out(ans[i + 1]);
