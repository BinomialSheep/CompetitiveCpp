...
class Task {
 public:
  int d, c, s;
  Task(int d_, int c_, int s_) : d(d_), c(c_), s(s_) {}
  bool operator<(Task& t) { return d < t.d; };
};
...
struct Solver {
  void solve() {
    ...
    rep(i, N) {
      INT(D, C, S);
      tasks.emplace_back(D, C, S);
    }
    // 締め切りが早い順にソート
    sort(all(tasks));
    for (auto t : tasks) debug(t.c, t.d, t.s);
    ...
    rep(i, N) {
      ...
      rep(j, 5001) {
        // 採用する場合
        int k = j + tasks[i].c;
        if (k <= tasks[i].d) chmax(p[k], dp[j] + tasks[i].s);
        // 不採用なら何もしない
        chmax(p[j], dp[j]);
      }
      swap(dp, p);
