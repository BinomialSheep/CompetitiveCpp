...
struct Solver {
  void solve() {
    ...
    auto f = [D](ll a) { return 1LL << (D - __builtin_popcountll(a)); };
    ...
    auto f2 = [D](ll a) {
      ...
      return 1LL << free_digits;
    };

    // for (ll i = 3; i <= 1e18; i = i * 2 + 1) assert(f(i) == f2(i));

    rep(bit, 1 << N) {
      ll tmp = 0;
      ll cnt = 1;
      rep(i, N) {
        if ((bit >> i) & 1) cnt *= -1, tmp |= A[i];
      }
      // debug(bit, cnt, tmp, f(tmp));
      ans += cnt * f(tmp);
    }
    out(ans);
