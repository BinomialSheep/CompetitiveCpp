...
struct Solver {
  void solve() {
    ...
    vvvl dp(N, vvl(N, vl(2)));
    ll INF = 1e18;
    rep(i, N) rep(j, N) dp[i][j][0] = -INF, dp[i][j][1] = INF;
    rep(i, N) dp[i][i][0] = a[i], dp[i][i][1] = -a[i];

    for (int k = 1; k < N; k++) rep(i, N) {
        int j = i + k;
        if (j >= N) break;
        if (j - 1 >= i) chmax(dp[i][j][0], dp[i][j - 1][1] + a[j]);
        if (i + 1 <= j) chmax(dp[i][j][0], dp[i + 1][j][1] + a[i]);
        if (j - 1 >= i) chmin(dp[i][j][1], dp[i][j - 1][0] - a[j]);
        if (i + 1 <= j) chmin(dp[i][j][1], dp[i + 1][j][0] - a[i]);
      }
    // debug(dp);
    out(dp[0][N - 1][0]);
