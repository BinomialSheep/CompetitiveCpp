...
struct Solver {
  void solve() {
    ...
    // 頂点集合V1の部分集合S1について、S1が独立集合かという配列
    vector<int> ok(1 << (N / 2));
    vector<int> ok2(1 << (N - N / 2));
    ...
    rep(S1, 1 << (N / 2)) {
      rep(i, N / 2) {
        ...
        if (ok[next]) continue;
        int flag = 1;
        rep(j, N / 2) if ((S1 >> j & 1) && G[i][j]) flag = 0;
        if (flag) ok[next] = 1;
      }
    }
    debug(ok);
    ...
    rep(S2, 1 << (N - N / 2)) {
      rep(i, N - N / 2) {
        ...
        int flag = 1;
        rep(j, N - N / 2) if ((S2 >> j & 1) && G[i + N / 2][j + N / 2]) flag =
            0;
        if (flag) ok2[next] = 1;
      }
    }
    ...
    vector<int> U(1 << (N / 2));
    U[0] = (1 << (N - N / 2)) - 1;
    rep(i, N / 2) rep(j, N - N / 2) {
      if (G[i][j + N / 2]) continue;
      U[i] |= 1 << j;
    }
    debug(U);

    // V2の部分集合Uの最大独立集合
    vector<int> dp(1 << (N - N / 2));
    rep(S2, 1 << (N - N / 2)) if (ok2[S2]) dp[S2] = S2;
    rep(S2, 1 << (N - N / 2)) {
      rep(i, N - N / 2) {
        int next = S2 | (1 << i);
        chmax(dp[next], dp[S2]);
