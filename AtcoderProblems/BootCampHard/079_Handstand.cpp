...
struct Solver {
  void solve() {
    ...
    rep(i, m) {
      ...
      int left = 0;
      int right = 0;
      if (i - 1 >= 0) left = vec[i - 1].second;
      if (i + 1 < m) right = vec[i + 1].second;
      chmax(ans, left + right + vec[i].second);
    }
    ...
    vi prefix(m + 1);
    rep(i, m) prefix[i + 1] = prefix[i] + vec[i].second;
    debug(prefix);
    ...
    rep(i, m) {
      ...
      debug(i, j, prefix[j] - prefix[i]);
      chmax(ans, prefix[j] - prefix[i]);
    }
    out(ans);
