...
struct Solver {
  void solve() {
    ...
    /* solve */
    // 正規表現でマッチしたやつを全部配列に詰める
    auto regexSearches = [&](regex re, string s) -> vector<string> {
      vector<string> ret;

      for (sregex_iterator it(s.begin(), s.end(), re), end; it != end; ++it) {
        match_results mr = (*it);
        ret.push_back(mr.str());
      }
      return ret;
    };

    regex re{R"(A+(A|BC)+)"};
    vs vec = regexSearches(re, S);
    // ランレングス圧縮して後ろからカウント
    ll ans = 0;
    for (auto t : vec) {
      stack<pii> st;
