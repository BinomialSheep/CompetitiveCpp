...
struct Solver {
  void solve() {
    ...
    rep(i, N) A[i].push_back(i);

    /* solve */
    vi ans(N, -1);
    set<int> decidedArea;

    while ((int)decidedArea.size() < N) {
      // functional graph
      vi B(N, -1);
      rep(i, N) rep(j, N) {
        if (ans[i] >= 0 || decidedArea.count(A[i][j])) continue;
        B[i] = A[i][j];
        break;
      }

      vi used(N, -1);
      auto dfs = [&](auto rec, int v, int s) -> int {
        used[v] = s;
        if (used[B[v]] == s) {
          ans[v] = B[v];
          decidedArea.insert(B[v]);
          return B[v];
        } else if (used[B[v]] == -1) {
          int res = rec(rec, B[v], s);
          // res == B[v]でループの最後まで復元完了
          if (res == B[v] || res == -1) {
            return -1;
          } else {
            ans[v] = B[v];
      ...
      rep(i, N) if (used[i] == -1 && ans[i] == -1) dfs(dfs, i, i);
    }
