...
int main() {
  ...
  // 別タスクとして、あるガチャを最初に引いたユーザーを覚えておく
  ...
  for (int i = 0; i < inputPairList.size(); i++) {
    ...
    if (itemMap.count(itemId) == 0) {
      // 初出のガチャなら新しい木を生成する
      thisGacha = unionFindTree.p.size();
      unionFindTree.p.push_back(thisGacha);
      unionFindTree.rank.push_back(thisGacha);

      unionFindTree.makeSet(thisGacha);
  ...
  */
  ...
  int gachaId = 1;
  for (auto gacha : gachaItemMap) {
    cout << gachaId << " " << firstUser[gacha.first] << endl;
    gachaId++;
  }
