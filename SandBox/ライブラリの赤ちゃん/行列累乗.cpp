...
struct Solver {
  void solve() {
    ...
    // 行列積
    auto matMul = [](vvl& A, vvl& B, ll mod = 4e18) {
    ...
    // 行列累乗 A^N
    auto matPow = [matMul](vvl& A, ll n, ll mod = 4e18) {
      vvl ret(A.size(), vl(A.size()));
      rep(i, A.size()) ret[i][i] = 1;  // 単位行列で初期化
      while (n > 0) {
        if (n & 1) ret = matMul(A, ret, mod);
        A = matMul(A, A, mod);
        n >>= 1;
      }
      return ret;
    ...
    // {fib(n+2), fib(n+1)} = (mat^n){fib2, fib1}
    mat = matPow(mat, N - 1, MOD);
    // {fib(n+2), fib(n+1)} = {{fib(n+1),fib(n)},{fib(n),fib(n-1)}}
    ll fibN = mat[0][1];
    // debug(mat);
